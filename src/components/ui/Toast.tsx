...
export function Toast ({ visible, className, ...props }: ToastProps): JSX.Element {
  return (
    <div
      className={cn(
...
export function toast (opts: ToastOpts): string {
  ...
  return hotToast.custom(
    ({ visible }) => (
      <Toast
        ...
        {message.length > 0 && <Toast.Description>{message}</Toast.Description>}
      </Toast>
    ),
    { duration }
  )
